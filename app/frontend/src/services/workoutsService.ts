...
const addCommentToWorkout = async (token: string, userId: number, programId: number, workoutId: number, comment: string) => {
    const url = `${import.meta.env.VITE_API_BASE_URL}/coaches/users/${userId}/user_programs/${programId}/user_program_workouts/${workoutId}/workout_comments`;
...
const updateWorkoutComment = async (token: string, commentId: number, userId: number, programId: number, workoutId: number, comment: string) => {
    const url = `${import.meta.env.VITE_API_BASE_URL}/coaches/users/${userId}/user_programs/${programId}/user_program_workouts/${workoutId}/workout_comments/${commentId}`;
    try {
        const response = await fetch(url, {
            method: 'PUT',
            headers: { 
...
const deleteWorkoutComment = async (token: string, commentId: number, userId: number, programId: number, workoutId: number) => {
    const url = `${import.meta.env.VITE_API_BASE_URL}/coaches/users/${userId}/user_programs/${programId}/user_program_workouts/${workoutId}/workout_comments/${commentId}`;
    try {
        const response = await fetch(url, {
            method: 'DELETE',
